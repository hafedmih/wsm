<form method="POST" class="d-block ajaxForm" action="<?php echo route('event_calendar/create'); ?>">
 ...
 <div class="form-group mb-1">
  ...
  <select class="form-control select2" data-toggle="select2" name="project_id" id="project_id">
      ...
      <?php $lang    = get_settings('language');
    ...
    'code !=' => 'pilote'
      ...
      foreach($projects as $project): 
          // Logic: If language is Arabic and title_ar is not empty, use it. Otherwise, use title.
          $display_title = ($lang == 'arabic' && !empty($project['title_ar'])) ? $project['title_ar'] : $project['title'];
      ?>
        <option value="<?php echo $project['id']; ?>"><?php echo $display_title; ?></option>
      <?php endforeach; ?>
  </select>
</div>
