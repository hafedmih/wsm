...
class Frontend_model extends CI_Model {
  ...
  public function register_new_donor() {
    ...
    $user_data['status']    = 0; // IMPORTANT: 0 means Pending Approval
    $user_data['watch_history'] = '[]';
    
    // Add school_id if your system is multi-tenant, otherwise set to 0 or 1
    $user_data['school_id'] = html_escape($this->input->post('school_id')); 

    $this->db->insert('users', $user_data);
   // $user_id = $this->db->insert_id();

    // B. Insert into DONORS table
//    $donor_data['user_id']        = $user_id;
//    $donor_data['name']           = $user_data['name'];
//    $donor_data['email']          = $user_data['email'];
//    $donor_data['contact_person'] = ''; // Can be updated later
...
//  
    ...
    $this->email_model->send_donor_welcome_email($name, $email, $plain_password);
     
    $admins = $this->db->get_where('users', ['role' => 'superadmin', 'status' => 1])->result_array();
    
    foreach ($admins as $admin) {
        $this->email_model->notify_admins_of_registration($admin['email'], $name, 'donor');
    }
}
public function register_ministry() {
        // 1. Check Duplication
        ...
        $data['address']   = html_escape($this->input->post('address')); // Using address field for Job Title
        $data['role']      = 'admin'; // <--- ROLE IS ADMIN
        ...
        // 4. Send Welcome Email (Optional: reuse the donor email logic or create new one)
        $this->load->model('email_model');
        // You might want to create a specific function for ministry emails later
        $this->email_model->send_ministry_welcome_email($data['name'], $data['email'], $this->input->post('password')); 

       $admins = $this->db->get_where('users', ['role' => 'superadmin', 'status' => 1])->result_array();
    
    foreach ($admins as $admin) {
        $this->email_model->notify_admins_of_registration($admin['email'], $data['name'], 'admin');
    }
