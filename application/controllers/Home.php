...
public function notice_details($notice_id = '') {
    // DEBUG: Uncomment the line below to see if the ID is actually changing
    // die("The ID received is: " . $notice_id);

    if (empty($notice_id)) {
        redirect(route('dashboard'), 'refresh');
    }

    // Use explicit where to ensure CodeIgniter doesn't ignore the parameter
    $this->db->where('id', $notice_id);
    $notice = $this->db->get('noticeboard')->row_array();
    
    if (empty($notice)) {
        $this->session->set_flashdata('error_message', get_phrase('notice_not_found'));
        redirect(route('dashboard'), 'refresh');
    }
    ...
    // 2. Call Model to create User & Donor
    //$this->load->model('user_model');
    $this->frontend_model->register_new_donor();

    // 3. Success Message
    $this->session->set_flashdata('flash_message', get_phrase('account_created_successfully_wait_for_admin_approval'));
...
public function register_ministry() {
    ...
    // 2. Call Model to create User & Donor
    //$this->load->model('user_model');
    $this->frontend_model->register_ministry();
